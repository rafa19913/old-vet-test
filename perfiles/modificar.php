...
<section id="content">
	...
	<?php
		...
		$perfil = $conexion->query('select * from perfil where id_perfil = '.$_POST['id'])->fetch_assoc();
	...
	float: left;
	border-radius: 5px;
	height: 20px;" value="<?php echo $perfil['nombre'] ?>"/>
        <br>
        <br>
		...
		while($opcion = $resul->fetch_assoc()){
			echo '<input type="checkbox" name="opciones[]"  value="'.$opcion['id_opcion'].'" ';
			
			
			$resul2 = $conexion->query('select * from menu where id_perfil = '.$perfil['id_perfil'].' and id_opcion = '.$opcion['id_opcion']);
			
			if($resul2->num_rows > 0)
			echo 'checked';
			
			echo '/>'.$opcion['nombre'].'<br>';			
		}
		?>
<br>

        
        <input type="submit" value="Guardar" style="width: 350px;
	font-size: 12px;
	padding: 2px 5px 7px 5px !important;
	background: #fff;
	outline: none;
