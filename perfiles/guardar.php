...
<section id="content">
    ...
    <div class="wrapper">
		...
		$query = 'update perfil set nombre = "'.$_POST['nombre'].'" where id_perfil = ' . $_POST['id'] ;
		...
		if($conexion->query($query)){
			...
			$conexion->query('delete from menu where id_perfil = '. $_POST['id']);
			for($i = 0; $i < count($opcion); $i++){
				$conexion->query('insert into menu values ('.$_POST['id'].', '.$opcion[$i].')');
			}
		}
		else
