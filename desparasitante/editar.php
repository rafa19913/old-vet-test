...
<section id="content">
    ...
    <div class="a1">
          <h2>Lista de Desparasitantes: </h2>
        </div>
    <div class="wrapper">
    <table>
            <tr>
        ...
        $conexion = conectar();
		$query = "select * from desparasitante";

		if(isset($_POST['buscar'])){
			$aux = $_POST['buscar'];
		$query = $query . ' where id_desparasitante like "' . $aux . '" or nombre like "%'.$aux.'%"' . ' or tipo like "%'.$aux.'%"';
		}
		//echo $query;
    ...
    echo '<tr>
      <td>'.$vacuna['id_desparasitante'].'</td>
      <td>'.$vacuna['nombre'].'</td>
      <td>'.$vacuna['producto_activo'].'</td>
      <td>'.$vacuna['tipo'].'</td>
	  <td>
	  <form action="modificar.php" method="post">
	  <input type="hidden" name="id" value="'.$vacuna['id_desparasitante'].'"/>
